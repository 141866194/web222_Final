...
*/
  ...
  if (isLoading) {
    document.querySelector("#search-button").innerHTML =
      '<i class="fa fa-hourglass"></i> Loading'; //Change button text
    document.querySelector("#search-button").disabled = true; //Disable button
  } else if (!isLoading) {
    document.querySelector("#search-button").innerHTML = "&#x1F50D; Search"; //Change button text back to normal
    document.querySelector("#search-button").disabled = false; //Enable button
  }
}
